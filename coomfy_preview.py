"""CoomfyLTX2SamplingPreview — live animated latent previews for LTX-2 sampling.

Derived from ComfyUI-KJNodes (https://github.com/kijai/ComfyUI-KJNodes),
file ``nodes/ltxv_nodes.py`` (``LTX2SamplingPreviewOverride``,
``OuterSampleCallbackWrapper``, ``prepare_callback``, ``WrappedPreviewer``,
``get_ltx_rgb_factors``), Copyright (C) kijai and contributors, licensed under
the GNU General Public License v3. This derivative file is therefore also
GPL-3.0; attribution is preserved per the license. Thank you, kijai!

Coomfy changes vs the original:

1. ``WrappedPreviewer.last_time`` is initialized at construction, removing the
   first-callback burst that decoded and sent *every* frame of the video at
   once (``last_time = 0`` bug upstream).
2. Preview decode/encode runs on a real background thread (upstream calls
   ``Thread(...).run()`` synchronously inside the sampling callback, stalling
   the sampler and ComfyUI's event loop).
3. The background thread re-enters ``torch.inference_mode()`` — thread-local
   context that upstream's synchronous code inherited for free.
4. A non-blocking lock serializes preview work: one in-flight chunk max,
   newer chunks are skipped so the preview stays real-time on long videos.
5. JPEG quality 70 instead of 95 — much cheaper encode on the server process.
6. The optional latent-upscale input is dropped (unused by Coomfy).

Wire format is kept identical to KJNodes' animated preview (4-byte event +
4-byte constant + 4-byte frame index + 16-byte pascal node id + JPEG) so
existing ComfyUI websocket preview consumers work unchanged.
"""
...
class _WrappedPreviewer:
    """Time-based animated preview: emits ``rate`` frames per wall-clock second."""
    ...
    def process_previews(self, image_tensor, ind, leng):
        try:
            with torch.inference_mode():
                self._process_previews_inner(image_tensor, ind, leng)
        finally:
            self._preview_lock.release()

    def _process_previews_inner(self, image_tensor, ind, leng):
        image_tensor = self.decode_latent_to_preview(image_tensor)

        if image_tensor.size(1) < _PREVIEW_MIN_SIZE or image_tensor.size(2) < _PREVIEW_MIN_SIZE:
            image_tensor = F.interpolate(
                image_tensor.movedim(-1, 0), scale_factor=4, mode="nearest"
            ).movedim(0, -1)

        if image_tensor.size(1) > _PREVIEW_MAX_SIZE or image_tensor.size(2) > _PREVIEW_MAX_SIZE:
            image_tensor = image_tensor.movedim(-1, 0)
            if image_tensor.size(2) < image_tensor.size(3):
                height = (_PREVIEW_MAX_SIZE * image_tensor.size(2)) // image_tensor.size(3)
                image_tensor = F.interpolate(image_tensor, (height, _PREVIEW_MAX_SIZE), mode="nearest")
            else:
                width = (_PREVIEW_MAX_SIZE * image_tensor.size(3)) // image_tensor.size(2)
                image_tensor = F.interpolate(image_tensor, (_PREVIEW_MAX_SIZE, width), mode="nearest")
        ...
        for preview in previews_ubyte:
            ...
            i.save(message, format="JPEG", quality=_PREVIEW_JPEG_QUALITY, compress_level=1)
            # send_sync already uses call_soon_threadsafe
    ...
    def decode_latent_to_preview(self, x0):
        if self.taeltx is not None:
            ...
            x0 = x0.unsqueeze(0).to(
                dtype=self.taeltx.first_stage_model.decoder[1].weight.dtype, device=device
            )
        ...
        latent_image = F.linear(
            x0.movedim(1, -1), self.latent_rgb_factors, bias=self.latent_rgb_factors_bias
        )
        return torch.sigmoid(latent_image)


def _prepare_callback(model, steps, shape=None, vae=None, rate=8, taeltx=False, num_keyframes=0, is_23=False):
    latent_rgb_factors, latent_rgb_factors_bias = get_ltx_rgb_factors(is_23)
    previewer = _WrappedPreviewer(
        latent_rgb_factors, latent_rgb_factors_bias, rate=rate, taeltx=vae if taeltx else None
    )
...
class _OuterSampleCallbackWrapper:
    def __init__(self, vae=None, preview_rate=8, taeltx=False):
        self.vae = vae
    ...
    def __call__(self, executor, noise, latent_image, sampler, sigmas, denoise_mask, callback, disable_pbar, seed, latent_shapes):
        ...
        new_callback = _prepare_callback(
...
class CoomfyLTX2SamplingPreview:
    """Patch an LTX-2 model so samplers stream animated latent previews."""

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "model": ("MODEL", {"tooltip": "The model to add the sampling preview override to."}),
                "preview_rate": (
                    "FLOAT",
                    {
                        "default": 8.0,
                        ...
                        "max": 60.0,
                        "step": 0.5,
                        "tooltip": "Preview frames per second of sampling time.",
                    },
                ),
            },
            "optional": {
                "vae": ("VAE", {"tooltip": "Tiny VAE (e.g. taeltx2_3) used to decode previews. Without it a latent2rgb fallback is used."}),
            },
        }
    ...
    RETURN_NAMES = ("model",)
    FUNCTION = "patch"
    CATEGORY = "Coomfy/preview"
    DESCRIPTION = "Overrides the LTX2 sampling preview so latent previews stream during sampling. Ported from KJNodes (GPL-3.0) with Coomfy fixes."

    def patch(self, model, preview_rate, vae=None):
        model = model.clone()
        taeltx = False
        if vae is not None and vae.first_stage_model.__class__.__name__ == "TAEHV":
            taeltx = True
        model.add_wrapper_with_key(
            comfy.patcher_extension.WrappersMP.OUTER_SAMPLE,
            "coomfy_sampling_preview",
            _OuterSampleCallbackWrapper(vae, preview_rate, taeltx),
        )
        return (model,)


def get_ltx_rgb_factors(is_23):
    """(latent_rgb_factors, bias) for LTX (is_23=False) or LTX2/LTXAV (is_23=True)."""
    if not is_23:
        latent_rgb_factors = [
...
NODE_CLASS_MAPPINGS = {
    "CoomfyLTX2SamplingPreview": CoomfyLTX2SamplingPreview,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "CoomfyLTX2SamplingPreview": "Coomfy LTX2 Sampling Preview",
}
